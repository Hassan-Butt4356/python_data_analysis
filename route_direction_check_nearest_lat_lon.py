...
for _, row in new_df.iterrows():
    ...
    for _, detail_row in filtered_df.iterrows():
        ...
        # Compute distance between Stop On location and each stop
        ...
        alight_distances.append((distance, detail_row['seq_fixed'], detail_row['ETC_STOP_ID'], detail_row['ETC_STOP_NAME']))
    
    # Find the nearest alight stop based on the minimum distance
    if alight_distances:
        nearest_alight_stop = min(alight_distances, key=lambda x: x[0])  # x[0] is the distance
        alight_seq = nearest_alight_stop[1]  # seq_fixed
        alight_stop_id = nearest_alight_stop[2]  # ETC_STOP_ID
        alight_stop_name = nearest_alight_stop[3]  # ETC_STOP_NAME
    else:
        alight_seq = 0
        alight_stop_id = None
        alight_stop_name = None
    
    # Update new_df with nearest alight stop details in new columns
...
#Previous code starts here works fine with out suffix _new in columns
...
#     # Update new_df with nearest alight stop details
#     new_df.loc[row.name, 'Alight_Seq'] = alight_seq
#     new_df.loc[row.name, stop_off_id_column[0]] = alight_stop_id
#     new_df.loc[row.name, stop_off_column[0]] = alight_stop_name
    
    # Optional: Logging for debugging
    # print(f"Row {row.name}: Board_Seq: {board_seq}, Alight_Seq: {alight_seq}, Alight ID: {alight_stop_id}, Alight Name: {alight_stop_name}")


# Prevoius Code ends here working Fine
...
new_df.drop(columns=['ROUTE_SURVEYEDCode_SPLITED',*stop_off_lat_lon_columns,*stop_on_lat_lon_columns,'SEQ_CHECK']).to_csv(f'reviewtool_{today_date}_{project_name}_ROUTE_DIRECTION_CHECkNewV2.csv',index=False)
