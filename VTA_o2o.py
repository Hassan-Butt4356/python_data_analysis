...
# To get  details about the time and day when surveys were conducted for the VTA_RAIL database use script 020_script.py
...
# if details_file(sheet_name="O2O_STOPS") available then have to use that other wise have to generate O2O_STOPS file using VTA_O2O_STOPS_Code script 
# detail_df=pd.read_excel('details_vta_CA_od_excel (1).xlsx',sheet_name='O2O_STOPS')
# detail_df=pd.read_csv('O2O_STOPS_VTA.csv')
detail_df=pd.read_csv('O2O_STOPS_VTA_01.csv')
...
# new_df=new_df[new_df['_INTERV_INIT']!='999']
...
new_df['TIME_BOARDED_Clean']=new_df['TIME_BOARDED'].apply(clean_string)

new_df['TIME PERIOD'] = new_df['TIME_BOARDED_Clean'].map(time_period_mapping)
...
# This below snippet used to update the seq_fixed value by double checking the process with

# for index, row in final_df.iterrows():
#     # Get the seq_fixed for board and alight stops
#     detail_board_seq = seq_df.loc[seq_df['ETC_STOP_ID'] == row['BOARD_STOP[CODE]'], 'seq_fixed'].values[0]
#     detail_alight_seq = seq_df.loc[seq_df['ETC_STOP_ID'] == row['ALIGHT_STOP[CODE]'], 'seq_fixed'].values[0]
    
#     # Update Board_Seq if different
#     if detail_board_seq != row['Board_Seq']:
#         final_df.at[index, 'Board_Seq'] = detail_board_seq
    
#     # Update Alight_Seq if different
#     if detail_alight_seq != row['Alight_Seq']:
#         final_df.at[index, 'Alight_Seq'] = detail_alight_seq
