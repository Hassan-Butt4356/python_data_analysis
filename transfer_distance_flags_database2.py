...
df1['FLAG_DESCRIPTION'] = ''

def check_lat_long(transfers, prefix, row):
    cols = [f'{prefix}_TRAN_{i}_ON_BUS_LAT' for i in range(1, transfers+1)] + \
           [f'{prefix}_TRAN_{i}_ON_BUS_LONG' for i in range(1, transfers+1)] + \
           [f'{prefix}_TRAN_{i}_OFF_BUS_LAT' for i in range(1, transfers+1)] + \
           [f'{prefix}_TRAN_{i}_OFF_BUS_LONG' for i in range(1, transfers+1)]
    return any(pd.isnull(row[col]) for col in cols)

def add_flag_description(row):
    flag_desc = []

    if '.5' in str(row['noOfTransferGPS']):
        # Check PREV_TRANSFERS
        if row['PREV_TRANSFERS'] == '(1) One':
            if check_lat_long(1, 'PREV', row):
                flag_desc.append("Preview Transfers Lat-Long is missing")
        elif row['PREV_TRANSFERS'] == '(2) Two':
            if check_lat_long(2, 'PREV', row):
                flag_desc.append("One or more Preview Transfers Lat-Long is missing")
        elif row['PREV_TRANSFERS'] == '(3) Three':
            if check_lat_long(3, 'PREV', row):
                flag_desc.append("One or more Preview Transfers Lat-Long is missing")
        elif row['PREV_TRANSFERS'] in ['(4) Four', '(4+) Four or more']:
            if check_lat_long(4, 'PREV', row):
                flag_desc.append("One or more Preview Transfers Lat-Long is missing")

        # Check STOP_ON_LAT and STOP_ON_LONG
        if pd.isnull(row['STOP_ON_LAT']) or pd.isnull(row['STOP_ON_LONG']):
            flag_desc.append("Boarding Lat-Long is missing")

        # Check STOP_OFF_LAT and STOP_OFF_LONG
        if pd.isnull(row['STOP_OFF_LAT']) or pd.isnull(row['STOP_OFF_LONG']):
            flag_desc.append("Alighting Lat-Long is missing")
        
        # Check NEXT_TRANSFERS
        if row['NEXT_TRANSFERS'] == '(1) One':
            if check_lat_long(1, 'NEXT', row):
                flag_desc.append("Next Transfers Lat-Long is missing")
        elif row['NEXT_TRANSFERS'] == '(2) Two':
            if check_lat_long(2, 'NEXT', row):
                flag_desc.append("One or more Next Transfers Lat-Long is missing")
        elif row['NEXT_TRANSFERS'] == '(3) Three':
            if check_lat_long(3, 'NEXT', row):
                flag_desc.append("One or more Next Transfers Lat-Long is missing")
        elif row['NEXT_TRANSFERS'] in ['(4) Four', '(4+) Four or more']:
            if check_lat_long(4, 'NEXT', row):
                flag_desc.append("One or more Next Transfers Lat-Long is missing")

    return ', '.join(flag_desc)


df1['FLAG_DESCRIPTION'] = df1.apply(add_flag_description, axis=1)

# Display the resulting DataFrame
print(df1[['FLAG_DESCRIPTION']].head(10))
df1.to_csv(f'reviewtool_{today_date}_{project_name}_Distance_Transfer_Flags_Database2.csv', index=False)
