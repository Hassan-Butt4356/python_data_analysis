...
wkend_overall_df=pd.read_excel('TUCSON_AZ_CR.xlsx',sheet_name='WkEND-RAIL')
# wkend_overall_df['LS_NAME_CODE']=wkend_overall_df['LS_NAME_CODE'].apply(edit_ls_code_column)
wkend_route_df=pd.read_excel('TUCSON_AZ_CR.xlsx',sheet_name='WkEND-RailTotal')
...
wkday_overall_df=pd.read_excel('TUCSON_AZ_CR.xlsx',sheet_name='WkDAY-RAIL')
# wkday_overall_df['LS_NAME_CODE']=wkday_overall_df['LS_NAME_CODE'].apply(edit_ls_code_column)
wkday_route_df=pd.read_excel('TUCSON_AZ_CR.xlsx',sheet_name='WkDAY-RailTotal')
...
with pd.ExcelWriter(f'reviewtool_{today_date}_{project_name}_RailRouteLevelComparison(Wkday & WkEnd)_Latest.xlsx') as writer:
    wkday_route_direction_df.drop(columns=['STATION_ID_SPLITTED','CR_Total','Total_DIFFERENCE']).to_excel(writer,sheet_name='WkDAY Route DIR Comparison',index=False)
    wkend_route_direction_df.drop(columns=['STATION_ID_SPLITTED','CR_Total','Total_DIFFERENCE']).to_excel(writer,sheet_name='WkEND Route DIR Comparison',index=False)
    ...
    weekday_raw_df[['id','Completed',route_survey_column[0],'ROUTE_SURVEYED',stopon_clntid_column[0],stopoff_clntid_column[0],time_column[0],time_period_column[0],'Day','ELVIS_STATUS']].to_excel(writer,sheet_name='WkDAY RAW DATA',index=False)
    weekend_raw_df[['id','Completed',route_survey_column[0],'ROUTE_SURVEYED',stopon_clntid_column[0],stopoff_clntid_column[0],time_column[0],time_period_column[0],'Day','ELVIS_STATUS']].to_excel(writer,sheet_name='WkEND RAW DATA',index=False)
